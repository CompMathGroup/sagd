...
void K_permeability(double* tetta, double* K_perm) //Проницаемость
{
	...
	for (m=1;m<M-1;m++)
	{
		K_perm[m]=(tetta[m]*tetta[m])/(1+tetta[m])/(1+tetta[m]);   //Задаю проницаемость как квадрат насыщенности. (На самом деле в тетта лежит отношение насыщеносттей, поэтому такая формула)
	}
}
...
void W_filtration(double* tetta, double* K_perm, double* W_fil ) //Вектор Фильтрации. (На границах задаем что он ноль)
{
	...
	for (m=1;m<M-2;m++)
	{
		W_fil[m]=K_perm[m]/etta*(ro_s-ro_l)*gravity;		//m+1/2
	}
}
